...
"✨ Brainstorm" in der Seitenleiste startet eine Session zweiter Art: kein Claude Code im Projekt, sondern ein Gesprächspartner (Opus 5, ohne Werkzeuge, eigener System-Prompt aus `sidecar/sidekick/prompts/brainstorm.md`). Man spricht per Tap über die Brille oder tippt; die Antworten werden wörtlich vorgelesen, kurz gehalten, mit genau einer Rückfrage pro Runde. Nach jeder Antwort pflegt der Partner unsichtbar einen Ideen-Stand (Titel, Einzeiler, Problem, Nutzer, MVP-Funktionen, Nicht-Ziele, Stack, Entscheidungen, offene Fragen, nächste Schritte, Bereitschaft 0 bis 100), den das UI im Tab "Idee" live zeigt. Ab achtzig schlägt der Partner selbst vor, das Projekt anzulegen.

"Projekt anlegen" erzeugt unter `projects.base_dir` (Standard `~\Projects`, wird angelegt) einen Ordner mit `README.md`, `CLAUDE.md`, `docs/SPEC.md`, `docs/PLAN.md` (Meilenstein 1 ist immer ein lauffähiges Grundgerüst), `docs/DECISIONS.md`, `docs/OPEN_QUESTIONS.md`, `docs/KICKOFF.md` und `docs/BRAINSTORM.md` (das Gespräch). Die Dokumente schreibt Opus 5 parallel, je Datei ein Aufruf; ein Fehlschlag nach Wiederholung wird zum Platzhalter, der Rest landet trotzdem. Danach `git init` mit Initial-Commit und auf Wunsch sofort eine Claude-Code-Session im Ordner, die den Kickoff-Prompt bekommt (Meilenstein 1 umsetzen, nur bei blockierenden offenen Punkten nachfragen). Einstellungen: `brainstorm.model`, `brainstorm.docs_model`, `brainstorm.speak_replies`, `brainstorm.auto_listen` (Mikro nach der Antwort automatisch wieder öffnen, Standard aus), `brainstorm.thinking`, `projects.base_dir`, `projects.git_init`, `projects.start_session_after_create`. Design: `docs/superpowers/specs/2026-09-08-brainstorm-design.md`.
...
- `Failed to start Claude Code: [WinError 50]` in der gepackten App hieß: claude.exe sollte den Stderr-Handle des Sidecars erben, was unter Tauri nicht geht. Seit 2026-09-08 bekommt jede `ClaudeAgentOptions` einen `stderr`-Callback, damit das SDK Stderr über eine eigene Pipe führt (`claude/utility.py`, `cli_stderr`). Aus einer Konsole (`uv run`) trat der Fehler nie auf.
